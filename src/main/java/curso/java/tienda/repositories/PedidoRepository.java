...
public interface PedidoRepository extends JpaRepository<Pedido, Integer>{
	...
	@Query(value="select SUM(pedidos.total)from pedidos INNER JOIN detalles_pedidos ON detalles_pedidos.id_pedido = pedidos.id INNER JOIN productos ON productos.id = detalles_pedidos.producto INNER JOIN categorias ON categorias.id = productos.categoria_id WHERE pedidos.estado='enviado' AND pedidos.fecha LIKE ?% GROUP BY(productos.categoria_id)", nativeQuery=true)
	List<String> totalPedidosPorCategoria(String fecha);
